...
namespace DPM{
  ...
  void Tissue2D::Disperse(){
    ...
    for(int i=0;i<NCELLS;i++){
        for(j=0;j<(int)cells[i].NV;j++){
            cells[i].Verticies[j][0] = cells[i].r0*(cos(2.0*M_PI*(j+1)/cells[i].NV)) + X[i];
            cells[i].Verticies[j][1] = cells[i].r0*(sin(2.0*M_PI*(j+1)/cells[i].NV)) + Y[i];
        }
    }
  }
  ...
  void Tissue2D::CLEulerUpdate(int nsteps, float dt){
    ...
    cl::Buffer gpuVerts(context,CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,  sizeof (std::array<float,2>)*NCELLS*maxNV,  allVerts.data());
    cl::Buffer gpuForces(context,CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR, sizeof (std::array<float,2>)*NCELLS*maxNV, allForces.data());
    cl::Buffer gpuNV(context,CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof (unsigned int)*NCELLS, NV.data());
...
//__kernel void RepulsionForceUpdate(__global float2* Verts, __global float2* Forces,__global int* NV,__global float* r0 ,int PBC, float L, float Kre){
    ...
    queue.enqueueReadBuffer(gpuVerts, CL_TRUE, 0, sizeof(std::array<float,2>)*NCELLS*maxNV, allVerts.data());
