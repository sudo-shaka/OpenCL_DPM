...
__kernel void AreaForceUpdates(__global float2 *Verts, __global float2 *Forces,
                               __global int *NV, __global float *a0,
                               __global float *Ka) {
  int ci = get_global_id(0);
  ...
  for (int vj = 0; vj < NV[ci]; vj++) {
    ...
    Area += 0.5 * ((Verts[idx_im1].x + Verts[idx].x) *
                   (Verts[idx_im1].y - Verts[idx].y));
  }
  barrier(CLK_LOCAL_MEM_FENCE);
  if (Area < 0.0) {
    Area = -Area;
  ...
  Forces[index].x += (Ka[ci] / sqrt(a0[ci])) * 0.5 * areaStrain *
                     (Verts[im1].y - Verts[ip1].y);
  Forces[index].y += (Ka[ci] / sqrt(a0[ci])) * 0.5 * areaStrain *
                     (Verts[im1].x - Verts[ip1].x);
}

__kernel void BendingForceUpdates(__global float2 *Verts,
                                  __global float2 *Forces, __global int *NV,
                                  __global float *Kb, __global float *a0,
                                  __global float *l0) {
  int ci = get_global_id(0);
  int vi = get_global_id(1);
  int NUM_VERTS = get_global_size(1);
...
__kernel void PerimeterForceUpdates(__global float2 *Verts,
                                    __global float2 *Forces, __global int *NV,
                                    __global float *Kl, __global float *a0,
                                    __global float *l0) {
  int ci = get_global_id(0);
  int vi = get_global_id(1);
  int NUM_VERTS = get_global_size(1);
...
__kernel void RepulsionForceUpdate(__global float2 *Verts,
                                   __global float2 *Forces, __global int *NV,
                                   __global float *r0, int PBC, float L,
                                   float Kre) {
  int ci = get_global_id(0);
  int vi = get_global_id(1);
  int NUM_VERTS = get_global_size(1);
...
__kernel void AttractionForceUpdate(__global float2 *Verts,
                                    __global float2 *Forces, __global int *NV,
                                    __global float *l0, int PBC, float L,
                                    float Kat) {
  int ci = get_global_id(0);
  int vi = get_global_id(1);
  int NUM_VERTS = get_global_size(1);
...
__kernel void EulerUpdate(__global float2 *Verts, __global float2 *Forces,
                          __global int *NV, float dt) {
  int ci = get_global_id(0);
  int vi = get_global_id(1);
  int NUM_VERTS = get_global_size(1);
  if (vi > NV[ci]) {
